...
def lexicographic_Qs(env, Q_state):
    ...
    best_individual_Q = np.max(scalarised_Qs(env, Q_state, [1.0, 0.0]))
    best_ethical_Q = -9999999
...
def choose_action(st, eps, q_table, env, infoQ):
    ...
    if np.random.random() <= eps:
        ...
        best_Individual = np.max(scalarised_Qs(env, q_table[st[0], st[1], st[2]], [1.0, 0.0]))
...
def q_LEXning(env, alpha=0.8, gamma=0.7):
    """
    ...
    for episode in range(1, max_episodes + 1):
        ...
        while not done and step_count < max_steps:
            ...
            update_q_table(Q, env, alpha, gamma, actions[0], state, new_state, reward)

            state = new_state
            done = dones[0]
        q = Q[10, 11, 8].copy()
        a = lexicographic_Qs(env, q)
        #print(q[a])
